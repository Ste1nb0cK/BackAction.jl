...
@testset "Pure States: Vectors Method" begin
    ...
    for k in 1:length(states_nojumps)
        @test identifystate(states_nojumps[:, k]) == identifystate(params.psi0)
    end
    ...
    # for traj in trajectories
    for traj in trajectories
        jumptimes = [click.time for click in traj]
        mindt = extrema(jumpdts)[1]
        jumptimes = cumsum(jumpdts)
        ...
        # Verify that evaluation just after jump works,
        # and that there are as many states as jumps
        states_afterjumps = BackAction.states_att(tgiven_a, jumptimes, labels, sys, params.psi0)
        njumps = size(traj)[1]
        nstates = size(states_afterjumps)[2]
        @test nstates == njumps
        for k in 1:nstates
        ...
        # and that there are as many states as jumps
        tgiven_b = jumptimes .- mindt / 2
        states_beforejumps = BackAction.states_att(tgiven_b, jumptimes, labels, sys, params.psi0)
        njumps = size(traj)[1]
        nstates = size(states_beforejumps)[2]
        @test nstates == njumps
        for k in 1:nstates
