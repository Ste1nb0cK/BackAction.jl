...
function _save_func_monitoring(u, integrator, cache_phi, counter, inner)
    ...
    # ForwardDiff.derivative!(cache_aux1, theta -> expv(auxt, affect!.Heff_par(theta), affect!.cache_state), affect!.theta0)
    # cache_phi .= expv(auxt, affect!.Heff, cache_phi)
    # cache_phi .+= cache_aux1
    cache_phi .= ForwardDiff.derivative(theta -> expv(auxt, affect!.Heff_par(theta), affect!.cache_state), affect!.theta0) #+ expv(auxt, affect!.Heff, cache_phi)
