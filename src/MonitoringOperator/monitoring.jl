...
function _output_func_monitoring(sol, i)
    ...
    jumpaffect!.cache_phi .= expv(auxt, jumpaffect!.Heff, jumpaffect!.cache_phi) +
                             ForwardDiff.derivative(theta -> expv(auxt, jumpaffect!.Heff_par(theta), jumpaffect!.cache_state),
        jumpaffect!.theta0)
